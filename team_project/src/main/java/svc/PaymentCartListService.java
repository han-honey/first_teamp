...
public class PaymentCartListService {
	
	public ArrayList<CartDTO> cartList(String sessionId) {
		System.out.println("PaymentCartListService - cartList() 메서드 호출");
		...
		cartList = cartDAO.selectCartList(sessionId);
	...
	public int cartTotalPrice(String sessionId) {
		System.out.println("PaymentCartListService - cartTotalPrice() 메서드 호출");
		int totalPrice = 0;
		...
		// => 파라미터 : 페이지번호(pageNum), 페이지당 게시물 수(listLimit)
		// => 리턴타입 : ArrayList<FileBoardDTO>(articleList)
		totalPrice = cartDAO.selectTotalPrice(sessionId);
